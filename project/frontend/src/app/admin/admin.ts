...
export class Admin implements OnInit {
      ...
      next: (updatedUser) => {
        if (updatedUser) {
          ...
          this.successMessage.set(
            `Updated role for ${user.username} successfully.`,
          );
  ...
  deleteUser(user: User) {
    ...
    if (
      confirm(
        `Are you sure you want to completely delete user ${user.username} and all their data?`,
      )
    ) {
      this.userService.deleteUser(user.id).subscribe({
        next: () => {
          ...
          this.successMessage.set(
            `Deleted user ${user.username} successfully.`,
          );
          setTimeout(() => this.successMessage.set(""), 3000);
        },
        error: (err) => {
