...
export class StudentService {
  ...
  getStudents(): Observable<Student[]> {
    return this.http.get<Student[]>(this.studentsUrl).pipe(
      tap((studentsList: Student[]) =>
        this.log(`size of the list = ${studentsList.length}`),
      ),
  ...
  addStudent(student: Student): Observable<Student> {
    return this.http.post<Student>(this.studentsUrl, student, httpOptions).pipe(
      tap((studentAdded: Student) =>
        this.log(`added student id=${studentAdded.id}`),
      ),
  ...
  updateStudent(student: Student, id: number): Observable<Student> {
    return this.http
      .put<Student>(`${this.studentsUrl}/${id}`, student, httpOptions)
      .pipe(
        // tap(_ => this.log(`updated student id=${student.id}`)), // same as the line below
        tap((studentUpdated: Student) =>
          this.log(`updated student id=${studentUpdated.id}`),
        ),
